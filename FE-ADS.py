...
#Molecules = [['CH4',-24.03376510], ['CO',-14.90940997], ['CO2',-23.17457153],["H2",-6.75664087],['H2O',-14.26411655],['N2',-16.77991136],['N2O',-21.57599354],['NH3',-19.54729190],['NO',-12.39971547],['NO2',-18.58681627],['O2',-9.99926374],['SO2',-17.01174353]] --> valores MolCaixaAds


def EnergyTotMoleLayer():
    Molecules = [['CH4',-24.03376571], ['CO',-14.90941020], ['CO2',-23.17457063],["H2",-6.75664086],['H2O',-14.26603276],['N2',-16.77991105],['N2O',-21.57599451],['NH3',-19.54928300],['NO',-12.39966878],['NO2',-18.58687406],['O2',-9.99925787],['SO2',-17.01187053]]
...
def center_mol_dir_z():
    ...
    print(len(positions))
    u_min=10
    for j in range(IndexInit,len(positions)):
        print(positions[j,2])
        if positions[j,2]<u_min:
       	    u_min=positions[j,2]
    return [0,0,u_min]
...
def colect_results(Mol, folder,root, roor_all,layer):
    ...
    if 'OUTCAR' in files:
        ...
        if Check ==True:
            ...
            # Descomentar duas linhas abaixo caso queira extrair as figuras
            CONTCAR_figs()
            run_fig(Mol,folder,root,root_all,layer)
...
def run_fig(Mol,Path, Root, root_all, layer):
        figssh = str(root_all)+'/'+'figs.sh'
        os.system('cp {} ./figs.sh'.format(figssh)) 
        os.system('chmod +x figs.sh')
        os.system('sh figs.sh')
        os.system('ls')
        os.system('jmol -x top.spt && mv top.png /run/media/natan/home-natan/estudos/faculdade/IC/MoS2/results/allfigures_new/{} '.format(Mol+'_'+layer+'_top.png'))
        os.system('jmol -x side.spt && mv side.png /run/media/natan/home-natan/estudos/faculdade/IC/MoS2/results/allfigures_new/{} '.format(Mol+'_'+layer+'_side.png'))
...
def navigate_dir(Molecules,root,root_all, layer):
    global j_pref
    out=[]
    os.chdir(root)
    if (recursive=='yes'):
        for Mol in Molecules:
            ...
            for folder in folders:
                os.chdir(folder)
                aux=colect_results(Mol, folder, root, root_all, layer)
                if aux==None:
                    pass
                else:
                    if 'E_rel()' in prop:
            ...
            for i in range(1,len(saida)):
                eads=float(saida[i][2])
                if eads<float(menor_eads):
                    menor_eads=eads
                    j_pref=i
                    
            #descomente a linha abaixo para exibir o sitio preferencial de cada molécula
            print(saida[j_pref][0], saida[j_pref][2])
...
###################
def main():
    global Molecules, recursive, S_defects, type_supercel, prop,  props, figures, root_all
    ...
    out=[]
    root_all='/run/media/natan/home-natan/estudos/faculdade/IC/MoS2/results/'

    root1='/run/media/natan/home-natan/estudos/faculdade/IC/MoS2/results/ads_pristine_pref'
    layer='pristine'
    os.chdir(root1)
    #Molecules=['H2', 'H2O', 'NH3','CH4','N2','CO','O2','N2O','CO2','NO','NO2','SO2']# Molecules that will be calculated the proprieties, e.g. the roots directories
    Molecules=['NO']
    recursive='yes'
    S_defects=0# Number of sulphur defects in the system. Set 0 to neither defects
    print('Pristine')
    pris = navigate_dir(Molecules,root1,root_all, layer)
    ...
    print('Defective far')
    far = navigate_dir(Molecules,root2,root_all, layer)
    ...
    recursive='yes'
    S_defects=1# Number of sulphur defects in the system. Set 0 to neither defects
    print('Defective close')
    close = navigate_dir(Molecules,root3,root_all, layer)
